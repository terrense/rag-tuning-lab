...
def chat(
    ...
    *,
    ...
    trace：可选的 rag_lab.trace.Trace，传入则自动累加本次调用的 token 用量（无侵入）。
    """
    load_dotenv()
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if trace is not None:                       # 无侵入记账：token 用量进 trace
                trace.add_usage(prov["model"], data.get("usage"))
...
def chat_stream(
    ...
    *,
    ...
    """流式版：SSE 逐块 yield 正文文本。用于 /ask/stream 的首 token 体验。

    推理模型的 <think> 段：流式下逐字剥离比较麻烦，这里用状态机——在还没遇到
    </think> 前的内容不吐出，遇到后才开始吐（多数情况下 think 在最前面）。
    """
    load_dotenv()
    alias = model or resolve_role(cfg, role or "generate")
    prov = _resolve(alias, cfg)
    ...
    with requests.post(
        ...
        for line in resp.iter_lines(decode_unicode=True):
            ...
            if not choices:
                if trace is not None and obj.get("usage"):
                    trace.add_usage(prov["model"], obj["usage"])
                continue
